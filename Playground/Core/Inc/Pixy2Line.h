...
struct Vector {
	void print() {
		...
		sprintf(buf, "vector: (%d %d) (%d %d) index: %d flags %d\r\n", m_x0,
				m_y0, m_x1, m_y1, m_index, m_flags);
		println(buf);
	}
...
struct Intersection {
	void print() {
		char buf[64];
		uint8_t i;
		sprintf(buf, "intersection: (%d %d)\r\n", m_x, m_y);
		println(buf);
		for (i = 0; i < m_n; i++) {
			sprintf(buf, "  %d: index: %d angle: %d\r\n", i,
					m_intLines[i].m_index, m_intLines[i].m_angle);
			println(buf);
		}
	}
...
struct Barcode {
	void print() {
		char buf[64];
		sprintf(buf, "Barcode: (%d %d), val: %d flags: %d\r\n", m_x, m_y,
				m_code, m_flags);
		println(buf);
	}
...
public:
	...
	int8_t getMainFeatures(uint8_t features = LINE_ALL_FEATURES, bool wait =
			true) {
		return getFeatures(LINE_GET_MAIN_FEATURES, features, wait);
	}

	int8_t getAllFeatures(uint8_t features = LINE_ALL_FEATURES,
			bool wait = true) {
...
int8_t Pixy2Line<LinkType>::getFeatures(uint8_t type, uint8_t features,
		bool wait) {
	int8_t res;
	...
	while (1) {
		...
		if (m_pixy->recvPacket() == 0) {
			if (m_pixy->m_type == LINE_RESPONSE_GET_FEATURES) {
				// parse line response
				for (offset = 0, res = 0; m_pixy->m_length > offset;
						offset += fsize + 2) {
...
int8_t Pixy2Line<LinkType>::setMode(uint8_t mode) {
	...
	*(int8_t*) m_pixy->m_bufPayload = mode;
	...
	m_pixy->sendPacket();
	if (m_pixy->recvPacket() == 0 && m_pixy->m_type == PIXY_TYPE_RESPONSE_RESULT
			&& m_pixy->m_length == 4) {
