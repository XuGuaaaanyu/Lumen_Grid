...
int8_t TPixy2<LinkType>::init(uint32_t arg) {
	...
	while (HAL_GetTick() - t0 < 5000) {
		...
		delayMicroseconds(5); // delay for sync
	}
	// timeout
...
template<class LinkType>
int8_t TPixy2<LinkType>::setCameraBrightness(
		uint8_t brightness) {
	uint32_t res;
...
template<class LinkType>
int8_t TPixy2<LinkType>::setServos(uint16_t s0,
		uint16_t s1) {
	uint32_t res;
	...
	*(int16_t*) (m_bufPayload + 2) = s1;
	...
	sendPacket();
	if (recvPacket() == 0 && m_type == PIXY_TYPE_RESPONSE_RESULT
			&& m_length == 4) {
		res = *(uint32_t*) m_buf;
		return (int8_t) res;
	} else
...
template<class LinkType>
int8_t TPixy2<LinkType>::setLED(uint8_t r, uint8_t g,
		uint8_t b) {
	uint32_t res;
...
template<class LinkType>
int8_t TPixy2<LinkType>::setLamp(uint8_t upper,
		uint8_t lower) {
	uint32_t res;
